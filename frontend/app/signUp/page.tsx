...
export default function SignUp() {
  ...
  const signupFn = useAuthStore((state) => state.signup);
      ...
      <div className="flex flex-col justify-center items-center w-full md:w-1/2 px-6 py-8">
        <div className="w-full max-w-md space-y-6">
          {isSkeletonLoading ? (
            ...
            <>
              <h2 className="text-primary font-extrabold text-3xl md:text-4xl text-center">Sign Up</h2>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col w-full space-y-4">
                ...
                >
                  {mutation.isPending ? <FaSpinner className="animate-spin text-xl" /> : "Create Account"}
                </button>
              </form>

              <p className="text-center text-black font-bold">
                Already have an account?{" "}
                <Link href="/" className="font-extrabold text-primary">Login</Link>
              </p>
            </>
          )}
        </div>
