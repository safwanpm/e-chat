...
// import {
//   ArrowLeft, Phone, Paperclip, Mic, Send,  MoreVertical,
// } from 'lucide-react';
...
export const ChatWindow = ({ contact, setView }: ChatWindowProps) => {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async (text: string) => {
      ...
      await sendMessage({ text, image: imageUrl });
    },
    onSuccess: () => {
      setNewMessage('');
      ...
      <div className="p-3 dark:border-gray-700 bg-white dark:bg-gray-800">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleSend}
            disabled={sendMessageMutation.isLoading}
            className={`p-3 rounded-full transition-colors ${newMessage.trim() || selectedFile
              ? 'bg-primary hover:text-gray-600 text-white'
              : 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
              }`}
          >
            {sendMessageMutation.isLoading ? (
              ...
              newMessage.trim() || selectedFile ? <Send size={18} /> : <Mic size={18} />
            )}
          </button>
        </div>
      </div>
